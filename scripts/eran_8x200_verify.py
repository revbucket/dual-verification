...
eran_8x200 = eu.load_eran_8x200()
if torch.cuda.is_available():
    eran_8x200 = eran_8x200.cuda()

for idx in range(args.start_idx, args.end_idx):
    print("Handling MNIST Example %s" % idx)

    elide_net, test_input = eu.setup_mnist_example(eran_8x200, idx, EPS, elide=True)
    print(elide_net)
    ...
    with torch.no_grad():
        decomp.manual_dual_ascent(2000, verbose=100)
        scrub_idxs(decomp)
        passing = False
        for seq in [{15:25, 13:25, 11:25}, {9:25, 7:25, 5:25, 3:25, 1:25}, {15:50, 13:50, 11:50}, {9:50, 7:50, 5:50}]:
