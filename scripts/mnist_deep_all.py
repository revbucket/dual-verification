...
def decomp_2d_mip(bin_net, test_input, preact_bounds, time_offset):
    ...
    decomp.merge_partitions(partition_dim={1: 2, 3: 2, 5: 2, 7: 16, 9: 20, 11:1})
...
deep_net = eu.load_mnist_deep_net()

for idx in range(args.start_idx, args.end_idx):
    print('Handling MNIST example %s' % idx)


    bin_net, test_input = eu.setup_mnist_example(deep_net, idx, args.eps)
