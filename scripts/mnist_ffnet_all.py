...
def decomp_2d_mip(bin_net, test_input, preact_bounds, time_offset):
    ...
    for i in range(10):
        last_val = decomp.dual_ascent(200, verbose=200, optim_obj=optim_obj, iter_start=i * 200)
        scheduler.step()
    ...
    for i in range(2):
        last_val = decomp.dual_ascent(10, verbose=True, optim_obj=optim_obj, iter_start=i * 5)
        scheduler.step()


    # And finally merge and evaluate here
    decomp.merge_partitions(partition_dim={1:16, 3: 16, 5: 16, 7: 32, 9:1})
